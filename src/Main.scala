...
object Main {
  def main(args: Array[String]) {
    ...
    if (args != null && args.length >= 3) {
      ...
      val nativeRDD = nativeDoc.map(_.split(",")).filter(arrs => arrs.length > 12).map(arrs => Row(arrs(0), arrs(1), arrs(2), arrs(3), arrs(4), arrs(5), arrs(6), arrs(7), arrs(8), arrs(9), arrs(10), arrs(11), arrs(12))).cache()
      val nativeDataFrame = sqlContext.createDataFrame(nativeRDD, nativeSchema).cache()

      nativeDataFrame.registerTempTable("native")

      val sourceDoc = sparkContext.textFile(args(1))
      val sourceFields = Array("sfzh", "xm", "xb", "csrq")
